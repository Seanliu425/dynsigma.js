...
export const applyFilters = (graph: Graph, filters: FiltersState) => {
  ...
  graph.forEachNode((node) => {
    ...
    const nodeClusters = graph.getNodeAttribute(node, "clusters") || [graph.getNodeAttribute(node, "cluster")];
    ...
    const clusterVisible = Object.keys(filters.clusters).length === 0 || 
      (Array.isArray(nodeClusters) ? 
        nodeClusters.some(cluster => filters.clusters[cluster] === true) :
        filters.clusters[nodeClusters] === true);
